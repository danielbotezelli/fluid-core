function [grad_phi_i, grad_phi_j] = gradgauss(Nx, Ny, phi, dx, dy, grad_phi_i, grad_phi_j)
for i = 2:Ny + 1
    for j = 2:Nx + 1
        ...
        if j == Nx + 1, phif_e = phi(i, j + 1); end
        if j == 2,      phif_w = phi(i, j - 1); end
        if i == Ny + 1, phif_n = phi(i + 1, j); end
        if i == 2,      phif_s = phi(i - 1, j); end
